    ...
    <div className="pb-32 md:pb-20 relative">
      ...
      <div className="relative mt-4 md:mt-6 mx-4 rounded-[32px] overflow-hidden min-h-[260px] md:h-[380px] shadow-xl z-10 group">
        ...
        {banners.map((banner, idx) => (
           ...
           >
              ...
              />
              {/* Overlay Gradient - Stronger gradient for text readability on smaller area */}
        ...
        <div className="absolute inset-0 p-8 md:p-12 z-20 flex flex-col justify-center items-start text-white">
            <div className="animate-in slide-in-from-left-8 duration-700 fade-in flex flex-col items-start gap-3">
               <div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-md px-3 py-1.5 rounded-full font-bold text-xs tracking-wide border border-white/30">
                  <span>{banners[currentSlide].icon}</span>
                  <span>手工传递幸福</span>
               </div>
               <h1 className="font-hand text-4xl md:text-5xl font-bold leading-[1.1] whitespace-pre-line drop-shadow-lg">
               ...
               <div className="flex gap-4 pt-4">
                 <Button onClick={() => navigate('/shop')} className="!bg-white !text-wooly-brown hover:!bg-wooly-pink-100 border-none shadow-lg py-2 px-6 text-sm md:text-base">
                   开始选购
                 </Button>
               </div>
            </div>
        ...
        <div className="absolute bottom-6 right-6 flex gap-2 z-30">
          {banners.map((_, idx) => (
            <button 
              ...
              className={`h-1.5 rounded-full transition-all duration-300 backdrop-blur-sm shadow-sm ${currentSlide === idx ? 'w-6 bg-white' : 'w-1.5 bg-white/40 hover:bg-white/60'}`}
            />
          ))}
        </div>
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 mt-12 md:mt-16">
        <div className="flex items-end justify-between mb-8">
          <div>
            <span className="text-wooly-pink-500 font-bold tracking-widest uppercase text-sm flex items-center gap-2">
              <Sparkles className="w-4 h-4" /> 新品上架
            </span>
            <h2 className="font-hand text-3xl md:text-4xl font-bold text-wooly-brown mt-1">新鲜出炉</h2>
          </div>
          <Link to="/shop" className="group text-wooly-brown font-bold hover:text-wooly-pink-500 flex items-center gap-2 bg-white px-4 py-2 rounded-full shadow-sm hover:shadow-md transition-all">
            查看全部 <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
          </Link>
        </div>
...
const Admin = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newProduct: Product = {
      ...
      image: `https://picsum.photos/seed/${Date.now()}/500/500`, // Mock image
      stock: 10,
      tags: []
    };
    addProduct(newProduct);
    setFormData({ title: '', price: '', category: Category.PLUSHIES, description: '' });
    setIsAdding(false);
    toast.success('商品已发布！');
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
         <div className="bg-white p-8 rounded-[32px] shadow-soft text-center max-w-sm w-full">
            ...
            <h2 className="font-hand text-2xl font-bold mb-4 text-wooly-brown">管理员暗号</h2>
            <form onSubmit={checkPasscode}>
              <input 
                type="password" 
                placeholder="输入暗号..." 
                className="w-full p-3 rounded-xl bg-gray-50 text-center font-bold tracking-widest text-lg outline-none focus:ring-2 focus:ring-wooly-pink-300 mb-4"
                value={passcode}
      ...
      {isAdding && (
        <div className="bg-white p-8 rounded-[32px] shadow-soft mb-10 animate-in slide-in-from-top-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 max-w-2xl">
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center text-gray-400 cursor-pointer hover:bg-gray-50 transition-colors">
            ...
            {products.map(p => (
              <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50">
                ...
                <td className="p-4 text-right pr-8">
                  <button onClick={() => deleteProduct(p.id)} className="text-red-400 hover:text-red-600 font-bold text-sm">删除</button>
                </td>
              </tr>
            ))}
          </tbody>
