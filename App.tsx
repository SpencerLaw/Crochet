    ...
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans text-slate-800">
      ...
      <aside className="w-full md:w-64 bg-white border-r border-slate-200 p-6 flex flex-col gap-8 shadow-sm">
        ...
        <nav className="flex flex-col gap-2">
          <Link to="/admin" className="flex items-center gap-3 p-3 rounded-lg bg-indigo-50 text-indigo-700 font-semibold">
            <LayoutDashboard className="w-5 h-5" /> 控制台
          </Link>
          <Link to="/" className="flex items-center gap-3 p-3 rounded-lg text-slate-500 hover:bg-slate-50 transition-colors">
            <Globe className="w-5 h-5" /> 返回前台
          </Link>
        </nav>

        <div className="mt-auto border-t pt-6">
           <button onClick={onLogout} className="flex items-center gap-3 p-3 w-full text-slate-400 hover:text-red-500 transition-colors">
